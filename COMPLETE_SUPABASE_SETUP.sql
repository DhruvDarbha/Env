-- Complete Supabase Setup for Halos Brand Data
...
-- 5. Create policies for access (drop first if they exist)
DROP POLICY IF EXISTS "halos_data_insert_policy" ON halos_data;
DROP POLICY IF EXISTS "halos_data_select_policy" ON halos_data;

CREATE POLICY "halos_data_insert_policy" ON halos_data
  FOR INSERT WITH CHECK (true);

CREATE POLICY "halos_data_select_policy" ON halos_data
  FOR SELECT USING (true);

-- 6. Insert 30 dummy data points near UPenn
...
-- 8. Show sample of inserted data
SELECT location_description, ripeness_score, latitude, longitude
FROM halos_data
ORDER BY ripeness_score DESC
LIMIT 5;